    ...
    <div class="chat-workspace">
      <aside class="conversation-panel" aria-label="历史会话">
        ...
        <el-scrollbar class="conversation-list">
          ...
          <template v-else>
            <button v-for="conversation in conversations" :key="conversation.id" class="conversation-item" :class="{active: activeConversationId === conversation.id}" type="button" @click="selectConversation(conversation.id)">
              ...
              <el-dropdown trigger="click" @command="handleConversationCommand($event, conversation)">
                <span class="conversation-more" role="button" tabindex="0" aria-label="会话操作" @click.stop @keydown.enter.stop>
            ...
            <el-button v-if="conversationPage.hasMore" text class="load-conversations" @click="loadConversations(conversationPage.nextCursor || undefined, true)">加载更多</el-button>
      ...
      <section class="chat-shell">
        <div ref="messageList" class="chat-messages" aria-live="polite" aria-relevant="additions text" @scroll="handleMessagesScroll">
          ...
          <div v-if="!messages.length" class="message assistant">
            ...
            <div>
              ...
              <div class="suggestions">
                <button v-for="item in suggestions" :key="item" type="button" :disabled="isAsking" @click="ask(item)">{{ item }}</button>
              </div>
            </div>
          </div>
          <div v-for="message in messages" :key="message.id" class="message" :class="[message.role, {error: message.error}]">
            <div v-if="message.role === 'assistant'" class="msg-avatar"><el-icon><MagicStick /></el-icon></div>
            <div>
              <div class="msg-bubble">
                <div v-if="message.role === 'assistant'" class="markdown-content" v-html="renderMarkdown(messageText(message))"></div>
                <template v-else>{{ message.text }}</template>
                <span v-if="message.role === 'assistant' && message.pending" class="streaming-cursor" aria-label="正在生成"></span>
                <span v-if="message.role === 'assistant' && message.citations.length" class="citation-note">PDF 引用 {{ message.citations.length }} 条</span>
              </div>
              <div v-if="message.role === 'assistant' && message.citations.length" class="citations" aria-label="原文引用">
                <div v-for="citation in message.citations" :key="citation.key" class="citation">
                  <b><el-icon aria-hidden="true"><Document /></el-icon>{{ citation.fileName }}</b>
        ...
        <form class="chat-input" @submit.prevent="ask(question)">
          <el-input v-model="question" placeholder="输入你的问题，例如：差旅报销标准是什么？" size="large" :disabled="isAsking" />
          <el-button type="primary" :icon="Promotion" native-type="submit" :loading="isAsking" :disabled="isAsking || !question.trim()">发送</el-button>
        </form>
      </section>
    </div>
  </div>
...
async function ask(rawQuestion: string) {
  ...
  const user: ChatMessage = {id: localMessageId--, conversationId, role: 'user', text, citations: []}
  const assistant: ChatMessage = {id: localMessageId--, conversationId, role: 'assistant', text: '', citations: [], pending: true}
  ...
  try {
    await streamChat({question: text, conversationId, stream: true}, {
      onCitations: items => {
      ...
      onDone: result => {
        ...
        if (!assistant.text.trim()) assistant.text = '未生成有效回答，请尝试调整问题后重试。'
        scrollToLatest()
      },
      onError: error => {
        assistant.pending = false
...
async function handleConversationCommand(command: 'rename' | 'delete', conversation: ChatConversation) {
  ...
  try {
    await ElMessageBox.confirm(`确定删除“${conversation.title}”吗？`, '删除会话', {type: 'warning', confirmButtonText: '删除', cancelButtonText: '取消'})
...
<style scoped>
.chat-history-page { min-width: 0; }
.chat-workspace { display: grid; grid-template-columns: 250px minmax(0, 1fr); min-height: 0; height: 100%; gap: 14px; }
.conversation-panel { min-width: 0; overflow: hidden; border: 1px solid var(--card-border, var(--line)); border-radius: var(--radius-lg, 8px); background: var(--card-bg, #fff); display: flex; flex-direction: column; }
.conversation-panel-head { min-height: 56px; padding: 10px 12px 10px 16px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--line); }
.conversation-panel-head b { color: var(--ink); font-size: 14px; }
.conversation-list { min-height: 0; flex: 1; padding: 8px; }
.conversation-item { width: 100%; min-height: 58px; display: flex; align-items: center; gap: 4px; padding: 8px 7px 8px 10px; border: 0; border-radius: var(--radius-md, 6px); color: var(--ink); text-align: left; background: transparent; cursor: pointer; }
.conversation-item:hover { background: var(--blue-50, #eef4ff); }
.conversation-item.active { color: var(--primary); background: var(--primary-soft, #e8efff); }
.conversation-item-main { min-width: 0; flex: 1; display: grid; gap: 4px; }
.conversation-item-main b { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 13px; font-weight: 600; }
.conversation-item-main small { color: var(--muted); font-size: 11px; }
.conversation-more { width: 28px; height: 28px; display: inline-flex; align-items: center; justify-content: center; border-radius: 4px; color: var(--muted); opacity: 0; }
.conversation-item:hover .conversation-more, .conversation-item.active .conversation-more, .conversation-more:focus { opacity: 1; }
.conversation-more:hover, .conversation-more:focus { color: var(--primary); background: rgba(49, 93, 214, .1); outline: none; }
.conversation-loading { padding: 22px 12px; color: var(--muted); font-size: 13px; text-align: center; }
.load-conversations { width: 100%; margin-top: 2px; }
.chat-workspace .chat-shell { min-width: 0; }
.history-load-more { display: flex; justify-content: center; padding-bottom: 12px; }
@media (max-width: 900px) { .chat-workspace { grid-template-columns: 208px minmax(0, 1fr); gap: 10px; } }
@media (max-width: 760px) { .chat-workspace { grid-template-columns: 1fr; } .conversation-panel { max-height: 154px; } .conversation-panel-head { min-height: 44px; } .conversation-list { padding: 5px 8px; } .conversation-item { min-height: 44px; } .conversation-item-main { display: flex; align-items: center; justify-content: space-between; gap: 8px; } .conversation-item-main small { flex: 0 0 auto; } }
</style>
