...
public class DashScopeService {
    ...
    /**
     * Streams DashScope's native OpenAI-compatible SSE deltas without buffering.
     */
    public void streamChat(String question, String context, Consumer<String> onDelta,
        ...
        Map<String, Object> body = Map.of(
                "model", chatModel,
                "messages", List.of(
                        Map.of("role", "system", "content", "你是企业知识助手，禁止编造信息。"),
    ...
    /** Reactive DashScope stream used by the WebFlux controller. */
    public Flux<String> streamChatFlux(String question, String context) {
        ...
        String prompt = "仅依据以下企业知识库上下文回答问题。每条已标记【来源:文档ID-页码】的上下文，"
                ...
                + "如果上下文不足，请明确说明未找到相关信息。\n上下文：\n" + (context == null ? "" : context)
                + "\n问题：" + (question == null ? "" : question);
        Map<String, Object> body = Map.of("model", chatModel,
                "messages", List.of(Map.of("role", "system", "content", "你是企业知识助手，禁止编造信息。"),
                        Map.of("role", "user", "content", prompt)),
        ...
        return webClient.post().uri("/chat/completions").contentType(MediaType.APPLICATION_JSON)
                ...
                .onStatus(status -> status.isError(), response -> Mono.error(new IllegalStateException("模型服务暂时不可用（HTTP " + response.statusCode().value() + "）")))
