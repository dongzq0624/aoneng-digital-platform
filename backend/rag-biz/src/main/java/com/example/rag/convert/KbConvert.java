 ...
 * 控制器使用的强类型 DTO 之间架桥，统一处理 Number / 时间戳 / 嵌套列表等松散类型的容错。
 */
@Mapper
public interface KbConvert {
    ...
    /**
     * 知识库行 Map → {@link KnowledgeBaseResponse}。
     * 字段名变更时需同步前端类型定义。
     */
    ...
    KnowledgeBaseResponse toBaseResponse(Map<String, Object> row);

    /** 批量转换知识库行。 */
    List<KnowledgeBaseResponse> toBaseResponses(List<Map<String, Object>> rows);

    /** 单条文档行 Map → {@link KnowledgeBaseDocumentResponse}。 */
    @Mapping(target = "id", source = "id", qualifiedByName = "longFromObject")
    ...
    @Mapping(target = "createdAt", source = "createdAt", qualifiedByName = "instantFromObject")
    @Mapping(target = "updatedAt", source = "updatedAt", qualifiedByName = "instantFromObject")
    KnowledgeBaseDocumentResponse toDocumentResponse(Map<String, Object> row);

    /** 批量转换文档行。 */
    List<KnowledgeBaseDocumentResponse> toDocumentResponses(List<Map<String, Object>> rows);

    @org.mapstruct.Named("stringFromObject")
    default String stringFromObject(Object value) {
        // 统一处理 null 与其他任意对象的字符串化。
    ...
    default List<Long> longListFromObject(Object value) {
        // 把 List<Number> 安全转换为 List<Long>，去重保留顺序。非列表视为空列表。
    ...
    default Instant instantFromObject(Object value) {
        // 兼容 Instant / OffsetDateTime / java.sql.Timestamp 三种时间戳。
        if (value == null) return null;
