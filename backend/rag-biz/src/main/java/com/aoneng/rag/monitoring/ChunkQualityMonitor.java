...
public class ChunkQualityMonitor {
    ...
    public void check() {
        try {
            jdbc.query("SELECT doc_id, identical_child_count, child_without_embedding_count, cross_page_parent_ratio "
                            + "FROM kb_chunk_quality_metrics "
                            + "WHERE identical_child_count > 0 OR child_without_embedding_count > 0 "
                            + "OR cross_page_parent_ratio >= 80",
                    (org.springframework.jdbc.core.RowCallbackHandler) rs -> log.warn("Chunk quality alert: docId={}, identicalChildren={}, childrenWithoutEmbedding={}, crossPageRatio={}%%",
                            rs.getLong("doc_id"), rs.getLong("identical_child_count"),
                            rs.getLong("child_without_embedding_count"), rs.getBigDecimal("cross_page_parent_ratio")));
