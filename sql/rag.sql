...
-- Default organization structure. Existing rows with the same ID are preserved.
INSERT INTO sys_dept (id, parent_id, name, ancestors, sort, status)
VALUES
    (1, 0, U&'\603B\7ECF\529E', '0,', 10, 1),
    (2, 0, U&'\6280\672F\652F\6301\4E2D\5FC3', '0,', 20, 1),
    (3, 0, U&'\8425\9500\4E2D\5FC3', '0,', 30, 1),
    (4, 0, U&'\7814\53D1\4E2D\5FC3', '0,', 40, 1),
    (5, 0, U&'\5236\9020\4E2D\5FC3', '0,', 50, 1),
    (6, 0, U&'\6D77\5916\9500\552E\90E8', '0,', 60, 1),
    (7, 0, U&'\804C\80FD\4E2D\5FC3', '0,', 70, 1),
    (8, 0, U&'\5DE5\827A\54C1\8D28\4E2D\5FC3', '0,', 80, 1)
ON CONFLICT (id) DO NOTHING;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_user_empno    ON sys_user(employee_no) WHERE deleted = FALSE;
CREATE UNIQUE INDEX IF NOT EXISTS uk_user_username ON sys_user(username)   WHERE deleted = FALSE;
CREATE INDEX IF NOT EXISTS idx_user_dept           ON sys_user(dept_id)    WHERE deleted = FALSE;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_permission_perms ON sys_permission(perms) WHERE perms IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS kb_knowledge_base (
    ...
    chunk_size      INT          NOT NULL DEFAULT 1200,  -- 父块 token 上限
...
CREATE TABLE IF NOT EXISTS kb_knowledge_base_dept (
...
CREATE INDEX IF NOT EXISTS idx_kb_dept_dept ON kb_knowledge_base_dept(dept_id);
COMMENT ON COLUMN kb_knowledge_base.visibility IS '可见范围：PRIVATE仅自己 / DEPT本部门 / ORG全公司 / PUBLIC公开';
CREATE INDEX IF NOT EXISTS idx_kb_owner  ON kb_knowledge_base(owner_id) WHERE deleted = FALSE;
CREATE INDEX IF NOT EXISTS idx_kb_dept   ON kb_knowledge_base(dept_id)  WHERE deleted = FALSE;
...
CREATE INDEX IF NOT EXISTS idx_doc_kb    ON kb_document(kb_id) WHERE deleted = FALSE;
CREATE INDEX IF NOT EXISTS idx_doc_parse ON kb_document(parse_status);
CREATE INDEX IF NOT EXISTS idx_doc_chunk ON kb_document(chunk_status);
...
CREATE INDEX IF NOT EXISTS idx_qa_user_time ON kb_qa_record(user_id, created_at DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_feedback_qa ON kb_feedback(qa_record_id);
...
-- =====================================================================
-- Chat history and retrieval evaluation tables
-- This script is the complete schema initializer for fresh databases.
-- =====================================================================
CREATE TABLE IF NOT EXISTS kb_conversation (
    id BIGSERIAL PRIMARY KEY,
    user_id BIGINT NOT NULL REFERENCES sys_user(id),
    title VARCHAR(120) NOT NULL DEFAULT U&'\65B0\4F1A\8BDD',
    selected_kb_ids BIGINT[], last_message_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    deleted_at TIMESTAMPTZ, legacy_qa_record_id BIGINT UNIQUE
);
CREATE INDEX IF NOT EXISTS idx_conversation_user_updated
    ON kb_conversation(user_id, last_message_at DESC, id DESC) WHERE deleted_at IS NULL;
...
CREATE TABLE IF NOT EXISTS kb_chat_message (
    ...
    sequence_no INT NOT NULL, role VARCHAR(16) NOT NULL,
    content TEXT NOT NULL DEFAULT '', status VARCHAR(16) NOT NULL DEFAULT 'PENDING',
    source_kb_ids BIGINT[], retrieved_chunk_ids BIGINT[], model_name VARCHAR(64),
    latency_ms INT, error_message VARCHAR(500),
...
CREATE INDEX IF NOT EXISTS idx_chat_citation_message
    ON kb_chat_message_citation(message_id, rank_no);
...
CREATE INDEX IF NOT EXISTS idx_qa_conversation_time
    ON kb_qa_record(conversation_id, created_at DESC) WHERE conversation_id IS NOT NULL;

INSERT INTO kb_conversation (user_id, title, selected_kb_ids, last_message_at, created_at, updated_at, legacy_qa_record_id)
SELECT q.user_id, LEFT(COALESCE(NULLIF(TRIM(q.question), ''), U&'\5386\53F2\95EE\7B54'), 120),
       q.kb_ids, q.created_at, q.created_at, q.created_at, q.id
FROM kb_qa_record q
WHERE q.conversation_id IS NULL
  AND NOT EXISTS (SELECT 1 FROM kb_conversation c WHERE c.legacy_qa_record_id = q.id);

UPDATE kb_qa_record q SET conversation_id = c.id
FROM kb_conversation c WHERE c.legacy_qa_record_id = q.id AND q.conversation_id IS NULL;
...
FROM kb_qa_record q JOIN kb_conversation c ON c.legacy_qa_record_id = q.id
WHERE NOT EXISTS (SELECT 1 FROM kb_chat_message m WHERE m.conversation_id = c.id AND m.sequence_no = 1);

INSERT INTO kb_chat_message (conversation_id, sequence_no, role, content, status, source_kb_ids,
                             retrieved_chunk_ids, model_name, latency_ms, qa_record_id, created_at, completed_at)
SELECT c.id, 2, 'ASSISTANT', COALESCE(q.answer, ''), 'COMPLETED', q.kb_ids,
       q.retrieved_chunk_ids, q.model_name, q.latency_ms, q.id, q.created_at, q.created_at
FROM kb_qa_record q JOIN kb_conversation c ON c.legacy_qa_record_id = q.id
ON CONFLICT (qa_record_id) DO NOTHING;

UPDATE kb_qa_record q SET assistant_message_id = m.id
FROM kb_chat_message m WHERE m.qa_record_id = q.id AND q.assistant_message_id IS NULL;
